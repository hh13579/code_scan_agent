...
def build_graph():
    ...
    graph.add_node("select_review_context", select_review_context)
    graph.add_node("review_diff_with_llm", review_diff_with_llm)
    graph.add_node("verify_review_findings", verify_review_findings)
    ...
    graph.add_edge("llm_triage", "select_review_context")
    graph.add_edge("select_review_context", "review_diff_with_llm")
    graph.add_edge("review_diff_with_llm", "verify_review_findings")
    graph.add_edge("verify_review_findings", "merge_review_findings")
...
class _FallbackGraph:
    def invoke(self, state: GraphState) -> GraphState:
        ...
        state = select_review_context(state)
        state = review_diff_with_llm(state)
        state = verify_review_findings(state)
